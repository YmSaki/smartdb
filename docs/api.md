...
* **システムキー**(`project_id` = NULL): ロールは常に `system`。プロジェクトの作成・一覧・削除・wipe(§Projects参照)に加え、**任意のアクティブなプロジェクトに対していつでも**`POST /projects/{project}/apikeys`・`DELETE /projects/{project}/apikeys/{id}`を呼び出せる(運用上の緊急アクセス手段。新規プロジェクト作成直後の初回キー発行に限定されているわけではない)。SQL実行・バックアップ・リストア等、データそのものに触れる操作を**直接**行うAPIは提供していないが、上記の理由によりシステムキー保持者は自分自身にプロジェクトの`admin`キーを発行することで、ワンステップでそれらに到達できる。システムキーはフリート管理者向けの最上位キーであり、この経路は意図した設計として許容している。
