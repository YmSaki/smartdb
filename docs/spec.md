...
* WAL運用
* Restore機能
* Restoreはデータベース本体の差し替えに加え、同ディレクトリに残る`-wal`/`-shm`サイドカーファイルも削除する。WAL運用下では、プロセスの異常終了（クラッシュ・OOM Kill等）によりこれらのファイルにリストア前のトランザクションが残存し得るため、そのまま放置すると次回接続時にリストア前の変更がリプレイされ、復元が実質無効化される・DBが破損する恐れがある。
